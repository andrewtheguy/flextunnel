...
//! 2. after [`RELAY_OUTAGE_REBUILD`] it resolves, telling the caller to
//!    replace the endpoint — the in-process equivalent of the restart that is
//!    known to fix it. The caller (`run_server`) closes the wedged endpoint,
//!    binds a fresh one with the same identity, and serves on again.
//!
...
/// How long from the start of the outage before the watchdog gives up on the
/// endpoint and asks for a rebuild. Leaves the nudge two minutes to take
/// effect (a net report through slow relays can take tens of seconds).
pub const RELAY_OUTAGE_REBUILD: Duration = Duration::from_secs(180);

/// Watch `endpoint`'s home-relay status and resolve — with the outage's
/// duration — once it has had no connected home relay for
/// [`RELAY_OUTAGE_REBUILD`], having nudged it with `network_change()` at
/// [`RELAY_OUTAGE_NUDGE`]. Never resolves while the home relay stays
/// connected; a reconnect at any point resets the clock. Pending forever once
/// the endpoint is gone.
pub async fn watch_home_relay(endpoint: &Endpoint) -> Duration {
    watch_outage(
...
/// (`Ok` = connected); `nudge` is the first-stage remedy.
async fn watch_outage<W, D, N, Fut>(mut watcher: W, describe: D, mut nudge: N) -> Duration
where
    W: Watcher,
    ...
    Fut: Future<Output = ()>,
{
    ...
    loop {
        ...
        let deadline = since + if nudged { RELAY_OUTAGE_REBUILD } else { RELAY_OUTAGE_NUDGE };
        tokio::select! {
...
mod tests {
    ...
    fn describe(connected: &bool) -> Result<(), String> {
        if *connected { Ok(()) } else { Err("down".into()) }
    }

    /// Run the watchdog on `status`, counting nudges. Returns the watchdog
    /// future's resolution wrapped in a bounded wait so a test never hangs.
    async fn run_for(
        status: &Watchable<bool>,
        nudges: Arc<AtomicUsize>,
        bound: Duration,
    ) -> Option<Duration> {
        let watchdog = watch_outage(status.watch(), describe, || {
    ...
    async fn healthy_relay_never_trips() {
        ...
        assert!(tripped.is_none(), "healthy relay must never request a rebuild");
    ...
    async fn sustained_outage_nudges_then_requests_rebuild() {
        ...
        let nudges = Arc::new(AtomicUsize::new(0));
        let elapsed = run_for(&status, nudges.clone(), RELAY_OUTAGE_REBUILD * 2)
            .await
            .expect("a sustained outage must request a rebuild");
        assert_eq!(nudges.load(Ordering::SeqCst), 1, "exactly one nudge before the rebuild");
        assert!(elapsed >= RELAY_OUTAGE_REBUILD);
        assert!(elapsed < RELAY_OUTAGE_REBUILD + Duration::from_secs(1));
    }
    ...
    async fn recovery_after_the_nudge_avoids_the_rebuild() {
        ...
        assert!(tripped.is_none(), "a relay that came back must not be rebuilt");
    ...
    async fn a_second_outage_starts_a_fresh_clock() {
        ...
        let elapsed = tripped.expect("second outage must eventually trip");
