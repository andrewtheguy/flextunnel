...
pub struct ClientConfig {
    ...
    /// `flextunnel generate-client-key`).
...
pub fn resolve_server(cli: ServerConfig, file: Option<ServerConfig>) -> Result<ResolvedServer> {
    ...
    // This avoids a false "both set" conflict when e.g. the CLI gives an inline
    // secret and the file gives a secret-file.
...
pub fn resolve_client(cli: ClientConfig, file: Option<ClientConfig>) -> ResolvedClient {
    ...
    // file's does. This avoids a false "both set" conflict when e.g. the CLI
    // gives an inline key and the config file gives a key file.
    let (auth_key, auth_key_file) = if cli.auth_key.is_some() || cli.auth_key_file.is_some() {
        (cli.auth_key, cli.auth_key_file)
    } else {
        (file.auth_key, file.auth_key_file)
    };
...
mod tests {
    ...
    fn parse_client_config() {
        let toml = r#"
            ...
            socks_port = 1085
            auth_key = "flextunnelsecretv1:AAAA"
